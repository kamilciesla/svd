...
SVDRec svdLAS2(SMat A, long dimensions, long iterations, double end[2], 
  ...
  //printf("Precision %d %d %d %d %d\n", ibeta, it, irnd, machep, negep);
  //2 53 2 -52 -53
  ...
  //for (int mm=0; mm<iterations + 1; ++mm) {
	//printf("*** %f\n", bnd[mm]);
  //}
   ...
   for (j=first; j<last; j++) {
	 ...
	 *ll = j;  
	 ...
	 oldeta[i] = eps1;
      ...
      purge(n, *ll, wptr[0], wptr[1], wptr[4], wptr[3], wptr[5], eta, oldeta, j, &rnm, tol);
      if (rnm <= tol) rnm = 0.0;
   }
   *rnmp = rnm;
   *tolp = tol;
...
void machar(long *ibeta, long *it, long *irnd, long *machep, long *negep) {
  ...
  volatile double beta, betain, betah, a, b, ZERO, ONE, TWO, temp, tempa, temp1;
  long i, itemp;
